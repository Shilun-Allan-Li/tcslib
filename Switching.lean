...
private lemma processClauseLits_sigma_indep_path {n : ℕ}
    (lits : List (Literal n × ℕ)) (path : List (Fin n × Bool))
    (ρ₀ σ₁ σ₂ : Restriction n) :
  ...
  | cons hd tl ih =>
    ...
    | cons p ps =>
      simp only [processClauseLits]
      have ⟨h1, h2, h3⟩ := ih ps (Function.update ρ₀ hd.1.var (some p.2))
        (Function.update σ₁ hd.1.var (some (!hd.1.neg)))
        (Function.update σ₂ hd.1.var (some (!hd.1.neg)))
      exact ⟨h1, h2, congrArg _ h3⟩

/-- The encoder's `.2` output (aux data) is independent of the `σ` parameter.
    Only `ρ₀`, `path`, `fuel`, and `acc` affect the aux entries. -/
private lemma encode_go_snd_sigma_indep {n : ℕ} (f : DNF n) (w : ℕ)
    (fuel : ℕ) (path : List (Fin n × Bool)) (ρ₀ σ₁ σ₂ : Restriction n)
    (acc : List (ℕ × Bool)) :
    (razborovEncode.go f w fuel path ρ₀ σ₁ acc).2 =
    (razborovEncode.go f w fuel path ρ₀ σ₂ acc).2 := by
  induction fuel generalizing path ρ₀ σ₁ σ₂ acc with
  ...
  | succ fuel ih =>
    ...
    | cons step rest =>
      ...
      · next t _ =>
        generalize hfli :
          List.filter (fun (x : Literal n × ℕ) => decide (x.1.var ∈ ρ₀.freeVars))
            (List.zipIdx t) = fli
        match fli with
        | [] => rfl
        | fl :: fls =>
          have ⟨h1, h2, h3⟩ := processClauseLits_sigma_indep_path
            (fl :: fls) (step :: rest) ρ₀ σ₁ σ₂
          simp only [h1, h2, h3]
          exact ih _ _ _ _ _
