  ...
  | succ k ih =>
    ...
    · split
      · -- some fixed
        rename_i _ h2
        simp only [DecisionTree.eval, restrictFn, DNF.eval]
        obtain ⟨t, ht_mem, ht_fixed⟩ := h2
        symm; rw [List.any_eq_true]
        refine ⟨t, ht_mem, ?_⟩
        show t.eval (ρ.extend x) = true
        rw [Term.eval, List.all_eq_true]
        exact fun l hl => Literal.fixedBy_eval_true l ρ (ht_fixed l hl) x
      · -- alive: find first non-killed clause, build complete sub-tree
        rename_i h1 h2
        split
        · -- find? = none: contradicts h1
          rename_i hfind
          exfalso; apply h1; intro t ht
          by_contra htk; rw [List.find?_eq_none] at hfind
          exact (hfind t ht) (by simp [htk])
        · -- find? = some t: complete sub-tree via termSubTree
          rename_i t hfind
          -- The sub-tree assigns all of t's free variables from x, then continues.
          -- By termSubTree_eval: eval = cont(ρ').eval where ρ' extends ρ at t's free vars.
          -- By termSubTree_extend_eq: ρ'.extend x = ρ.extend x.
          -- So restrictFn f.eval ρ' x = restrictFn f.eval ρ x.
          -- The cont checks fixedBy t ρ':
          --   If fixed: leaf true = 1 = f.eval(ρ.extend x) (since t is a satisfied disjunct)
          --   If not fixed: go f k ρ' = restrictFn f.eval ρ' x = restrictFn f.eval ρ x (by IH)
          sorry
        ...
        | fl :: fls =>
          ...
          case hE' => -- ∀ v, pcl.2.1 v = none → σ v = none
            intro v hv'; exact hE v (by
              by_contra h; exact absurd hv' (processClauseLits_rho_ne_none _ _ _ _ _ h))
          ...
          case hD' => -- ∀ v, pcl.2.1 v ≠ none → ρ₀_fold v = pcl.2.1 v
            ...
            · have hne := processClauseLits_aux_ne_nonfree t_clause _ _ ρ₀ σ v hmem_zip
          ...
          case hfuel' => -- df ≥ (encode ... σ []).2.length + 1
            rw [← hsigma_indep, haux] at hfuel
            simp [hpcl_cons, List.length_append] at hfuel ⊢; omega
          case henc' => rfl
          case hbase' =>
            intro heq
            -- If the recursive encoder returns (σ, []), decode on [] returns σ_fold
            -- and we need σ_fold = σ. Use roundtrip_base.
            have hE' : ∀ v, pcl.2.1 v = none → σ v = none := by
              intro v hv'; exact hE v (by
                by_contra h; exact absurd hv' (processClauseLits_rho_ne_none _ _ _ _ _ h))
            have hC'_fold : ∀ v, pcl.2.1 v ≠ none → σ_fold v = σ v := by
              ...
              · exact (foldl_sigma_stable t_clause _ _ _
                  (processClauseLits_aux_ne_nonfree t_clause _ _ ρ₀ σ v hmem_zip
                    (fun p hp h => hv (h ▸ hfree_lits p hp)))).trans (hC v hv)
            -- When enc' = (σ, []), enc'.1 = σ, so hA' reduces to σ_fold v = σ v
            have hA'_base : ∀ v, pcl.2.1 v = none → σ_fold v = σ v := by
              intro v hv'; simp only [σ_fold_def]
              have hv : ρ₀ v = none := by
                by_contra h; exact absurd hv' (processClauseLits_rho_ne_none _ _ _ _ _ h)
              have hne := processClauseLits_aux_ne_nonfree t_clause _ _ ρ₀ σ v hmem_zip
                (fun p hp h' => by
                  have := hfree_lits p hp; rw [h'] at this
                  exact absurd hv' (this ▸ processClauseLits_rho_ne_none _ _ ρ₀ σ v
                    (by rw [this]; exact fun h => by simp at h)))
              rw [foldl_sigma_stable t_clause _ _ _ hne, hA v hv, hγ]
              exact congrFun (congrArg _ (congrArg Prod.fst heq)) v
            exact roundtrip_base f w pcl.2.1 σ σ_fold ρ₀_fold df hE' hA'_base hC'_fold
...
private lemma fiber_bound {n : ℕ} (f : DNF n) (w s d : ℕ)
  ...
  -- The Razborov encoding ρ ↦ (γ, aux) is injective on bad restrictions
  -- in this fiber. Since .1 = γ is fixed, the .2 component (aux data)
  -- determines ρ uniquely. The aux data has d non-marker entries, each
  -- from {0,…,w−1} × Bool (2w values), giving at most (2w)^d fibers.
  --
  -- Injectivity proof sketch (does not need hnd):
  -- If ρ₁ ≠ ρ₂ in the fiber, they differ at some variable v.
  -- By encode_go_fst_nonfree, v must be free in one (say ρ₁) but not
  -- the other. At the encoding step processing v, the free-literal
  -- list for ρ₁'s clause includes v's literal but ρ₂'s does not,
  -- making the aux data (.2) differ structurally.
  sorry
