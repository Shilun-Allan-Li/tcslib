...
\[
\lvert B_t(p)\rvert = \text{pauli_error_ball_size }t
...
\[
\text{quantum_distance S} \le \text{pauliWeight p}.
\]
\end{lemma}
...
\[
\text{quantum_distance S} \le \mathrm{wt}(p-q).
\]
\end{lemma}
...
\leanok
If $d \le \text{quantum_distance S}$, then under the same hypotheses,
\[
d \le \mathrm{wt}(p-q).
\]
\end{lemma}
...
Assume:
\begin{itemize}
  \item $k = \text{quantum_code_dimension,S.S}$;
  \item a distance parameter $d>0$ with $d\le n$ and $d\le \text{quantum_distance S}$;
