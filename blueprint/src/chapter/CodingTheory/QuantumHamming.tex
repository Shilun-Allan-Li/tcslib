...
\[
(\alpha^n)\times(\alpha^n) = \lean{SympVec n α}.
\]
...
k := n - \dim_\alpha(S),
\]
where $S \le \lean{SympVec n α}$ is the stabilizer submodule and $\dim_\alpha(S)$ is
...
For $t\in\mathbb{N}$ and center $p\in\lean{SympVec n α}$, the error ball is the finite set
\[
...
\[
\lvert B_t(p)\rvert=\lean{pauli_error_ball_size}\ t
...
\[
\lean{quantum_distance S} \le \lean{pauliWeight}\ p.
\]
\end{lemma}
...
\[
\lean{quantum_distance S} \le \mathrm{wt}(p-q).
\]
\end{lemma}
...
\leanok
If $d \le \lean{quantum_distance S}$, then under the same hypotheses,
\[
d \le \mathrm{wt}(p-q).
\]
\end{lemma}
...
\section{Auxiliary inequality: triangle inequality for Pauli weight}
...
Assume:
\begin{itemize}
  \item $k = \lean{quantum_code_dimension}\ S.S$;
  \item a distance parameter $d>0$ with $d\le n$ and $d\le \lean{quantum_distance S}$;
...
\begin{enumerate}
  \item Compute the ball size \lean{pauli_error_ball_card} and rewrite it as the closed form sum.
  ...
  \item Bound the size of the syndrome space using finite-dimensional linear algebra, yielding
        $\lvert S.S \toₗ \alpha\rvert = q^{n-k}$ (the code constraints).
