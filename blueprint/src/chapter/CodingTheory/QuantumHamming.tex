...
\lean{TCSlib.CodingTheory.quantum_code_dimension}
...
\lean{TCSlib.CodingTheory.pauli_error_ball}
...
\lean{TCSlib.CodingTheory.pauli_error_ball_size}
...
\lean{TCSlib.CodingTheory.pauli_error_ball_size_independent}
\leanok
...
\lean{TCSlib.CodingTheory.pauli_error_ball_card}
\notready
...
\]
In Lean, the left-hand side is \lean{(pauli_error_ball t p).card} and the right-hand side is
\lean{pauli_error_ball_size t}.
...
\lean{TCSlib.CodingTheory.quantum_distance}
...
\lean{TCSlib.CodingTheory.quantum_distance_le_pauliWeight}
\leanok
If $p$ commutes with the stabilizer, $p\notin S$, and $\mathrm{wt}(p)\neq 0$, then
\[
\text{quantum distance S} \le \text{pauliWeight p}.
\]
\end{lemma}
...
\lean{TCSlib.CodingTheory.symplecticForm_sub_left}
\leanok
\[
...
\lean{TCSlib.CodingTheory.commutesWithStabilizer_sub}
\leanok
...
\lean{TCSlib.CodingTheory.quantum_distance_le_weight_sub}
\leanok
If $p,q$ commute with the stabilizer, and $p-q\notin S$ with nonzero weight, then
\[
\text{quantum distance S} \le \mathrm{wt}(p-q).
\]
\end{lemma}

\begin{lemma}[Convenient inequality form]
\lean{TCSlib.CodingTheory.pauliWeight_sub_ge_of_commuting}
\leanok
If $d \le \text{quantum distance S}$, then under the same hypotheses,
\[
d \le \mathrm{wt}(p-q).
\]
\end{lemma}
...
\lean{TCSlib.CodingTheory.pauli_error_ball_disjoint}
\notready
...
\lean{TCSlib.CodingTheory.pauliWeight_triangle}
\notready
...
\lean{TCSlib.CodingTheory.quantum_hamming_bound}
\notready
Let $S$ be a stabilizer code on $n$ qudits over $\alpha$ with $q=\lvert\alpha\rvert>1$.
Assume:
\begin{itemize}
  \item $k = \text{quantum code dimension,S.S}$;
  \item a distance parameter $d>0$ with $d\le n$ and $d\le \text{quantum distance S}$;
