...
\lean{CodingTheory.symplecticBilin}
...
\lean{CodingTheory.symplecticForm_add_left}
\leanok
\[
...
\lean{CodingTheory.symplecticForm_add_right}
\leanok
\[
...
\lean{CodingTheory.symplecticForm_smul_left}
\leanok
\[
...
\lean{CodingTheory.symplecticForm_smul_right}
\leanok
\[
...
\lean{CodingTheory.symplecticForm_sum_left}
\leanok
\[
...
\lean{CodingTheory.symplecticForm_sum_right}
\leanok
\[
...
\lean{CodingTheory.symplecticForm_smul_sum_left}
\leanok
\[
...
\lean{CodingTheory.symplecticDual}
For a submodule $S \le \lean{SympVec n α}$, define
\[
...
\lean{CodingTheory.stabilizer_is_isotropic}
\leanok
...
\lean{CodingTheory.stabilizer_le_symplecticDual}
\leanok
...
\lean{CodingTheory.ErasureCorrectable}
...
\lean{CodingTheory.erasure_correctable_of_dist}
\leanok
If $|E| < \lean{quantum_distance S}$, then $E$ is erasure-correctable.
The proof uses:
\begin{itemize}
  \item support $\Rightarrow$ weight bound $\mathrm{wt}(p)\le |E|$;
  \item if $p\notin S$ and $p$ commutes with $S$, then $\lean{quantum_distance S}\le \mathrm{wt}(p)$
        (via \lean{quantum_distance_le_pauliWeight});
  \item contradiction with $|E|<\lean{quantum_distance S}$.
\end{itemize}
\end{lemma}
...
The file introduces:
\begin{itemize}
  \item \lean{restrictTo p erased}: zeroes out coordinates outside the erased set;
  \item \lean{symplecticFormOn C p q}: symplectic form summed only over coordinates in $C$;
...
\lean{CodingTheory.symplecticFormOn_restrict}
\leanok
\[
...
\lean{CodingTheory.restrict_logical_to_erased_in_stabilizer}
\notready
...
\lean{CodingTheory.clean_logical_operator}
\notready
...
\lean{CodingTheory.supportSubmodule}
...
\subsection{Explicit linear equivalence to $(C\to \alpha)\times (C\to \alpha)$}
...
\lean{CodingTheory.finrank_supportSubmodule}
\leanok
\[
\dim_\alpha(V_C) = 2\,|C|.
\]
...
\lean{CodingTheory.symplecticForm_decompose}
\notready
...
\lean{CodingTheory.symplecticForm_nondegenerate}
\leanok
...
\lean{CodingTheory.finrank_symplecticDual}
\notready
For finite-dimensional ambient space,
\[
\dim(S^{\perp_\omega}) = 2n - \dim(S).
\]
The proof packages the bilinear form as a linear map into the dual space, uses
nondegeneracy to show injectivity, and relates $S^{\perp_\omega}$ to a dual annihilator.
\end{lemma}

\begin{lemma}[Logical space dimension identity]
\lean{CodingTheory.finrank_logical_space}
\notready
For a stabilizer $S$,
\[
\dim(S^{\perp_\omega}) - \dim(S) = 2\,(n-\dim(S)) = 2k,
\]
where $k=\lean{quantum_code_dimension S.S}$.
\end{lemma}

\section{Symplectic bases}

\begin{definition}[Symplectic basis in a submodule]
\lean{CodingTheory.SymplecticBasis}
...
\lean{CodingTheory.rank_bound_from_commutators_simple}
\notready
...
k \le |C|.
\]
The argument is: the $2k$ vectors are linearly independent inside the support submodule
$V_C$, hence
\[
2k = \dim(\mathrm{span}\{X_i,Z_i\}) \le \dim(V_C) = 2|C|,
\]
so $k\le |C|$.
\end{lemma}
...
\lean{CodingTheory.quantum_singleton_bound_of_rank}
\leanok
Assuming
\[
2(d-1) \le \dim(S),
...
\lean{CodingTheory.stabilizer_rank_ge_of_distance}
\notready
Assuming $d\le \lean{quantum_distance S}$ and a hypothesis \lean{h_basis} that produces
symplectic pairs $(X_i,Z_i)$ with controlled support after choosing disjoint sets
$A,B$ of size $d-1$ and $C=(A\cup B)^c$, one shows
\[
2(d-1) \le \dim(S).
\]
This is the technical core reducing distance to a rank constraint using the
commutator rank bound on the remaining coordinates.
\end{lemma}

\subsection{Main theorem}

\begin{theorem}[Quantum Singleton bound]
\lean{CodingTheory.quantum_singleton_bound}
\notready
Under the standard parameter assumptions ($0<d\le n$ and $d\le \lean{quantum_distance S}$)
