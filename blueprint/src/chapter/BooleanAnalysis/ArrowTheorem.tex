...
\begin{enumerate}
  \item Acyclicity $\Rightarrow$ $\mathrm{corr}(f) = -1/3$
        (Lemma~\texttt{ArrowTheorem.acyclic\_implies\_corrFunc}).
  \item $\mathrm{corr}(f) = -1/3$ $\Rightarrow$ $\hat f(S) = 0$ for $|S|\ne 1$
        (Lemma~\texttt{ArrowTheorem.corrFunc\_eq\_neg\_third\_of\_weight\_one}).
  \item Degree-1 + unanimous + $\pm 1$-valued $\Rightarrow$ dictator
        (Lemma~\texttt{ArrowTheorem.degree\_one\_implies\_dictator}).
\end{enumerate}
\end{theorem}
