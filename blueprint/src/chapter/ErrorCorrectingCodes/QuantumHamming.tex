\chapter{Quantum Hamming Bound}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Overview}

This chapter proves the \textbf{quantum Hamming bound} for non-degenerate
qubit stabilizer codes.  Working in the Pauli-string formalism, it shows that
if a quantum code $C \le \mathcal{H}_n$ satisfies the Knill--Laflamme
conditions for $t$-error correction and is non-degenerate, then
\[
\left(\sum_{i=0}^{t}\binom{n}{i}3^i\right)\cdot\dim_{\mathbb{C}}(C) \;\le\; 2^n.
\]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Pauli strings}

\begin{definition}[Pauli basis]
\lean{PauliBasis}
\statementsource{quant-ph.0005008}{
  pdf-1ac7bb3b99cc-p002-b006,
  pdf-1ac7bb3b99cc-p003-b001
}
\label{PauliBasis}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
The four Pauli basis elements $\{I,X,Y,Z\}$, represented as an inductive type.
\end{definition}

\begin{definition}[Pauli string]
\lean{PauliString}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b004}
\label{PauliString}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
A \emph{Pauli string} of length $n$ is a function $p : \mathrm{Fin}\,n \to \mathrm{PauliBasis}$.
\end{definition}

\begin{definition}[Support and weight]
\lean{support}
\label{support}
\statementsource{quant-ph.9705052}{pdf-e658890bd7ef-p002-b002}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b005}
\statementsource{quant-ph.0005008}{
  pdf-1ac7bb3b99cc-p002-b004,
  pdf-1ac7bb3b99cc-p004-b003
}
\lean{weight}
\label{weight}
\leanok
\statementsource{quant-ph.0508070}{pdf-4bcdc0ee9c86-p003-b007}
The \emph{support} $\mathrm{supp}(p) \subseteq \mathrm{Fin}\,n$ consists of coordinates
where $p(i) \neq I$; the \emph{weight} is $\mathrm{wt}(p) = |\mathrm{supp}(p)|$.
\end{definition}

\begin{definition}[Pauli error set]
\lean{PauliErrorsLe}
\label{PauliErrorsLe}
\leanok
\statementsource{PhysRevLett.77.2585}{
  pdf-9634739c3de9-p003-b003,
  pdf-9634739c3de9-p002-b004
}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p011-b008}
\statementsource{PhysRevA.55.900}{pdf-bbdd8e5c3949-p008-b003}
\uses{PauliString, weight}
The set of all Pauli strings of weight at most $t$:
\[
\mathcal{E}(n,t) = \bigl\{p : \mathrm{PauliString}\,n \;\big|\; \mathrm{wt}(p) \le t\bigr\}.
\]
\end{definition}

\begin{theorem}[Cardinality of the bounded-weight Pauli error set]
\lean{card_pauliErrorsLe}
\label{card_pauliErrorsLe}
\leanok
\proofsource{quant-ph.9705052}{pdf-e658890bd7ef-p010-b003}
\proofsource{PhysRevA.54.3824}{
  pdf-c11491661f48-p020-b001,
  pdf-c11491661f48-p021-b003
}
\proofsource{PhysRevLett.77.2585}{pdf-9634739c3de9-p003-b003}
\uses{PauliErrorsLe, weight}
\difficulty{5}
For natural numbers $n$ and $t$, the number of Pauli strings of length $n$ whose weight
is at most $t$ is
\[
|\mathcal{E}(n,t)| \;=\; \sum_{j=0}^{t} \binom{n}{j}\, 3^{\,j}.
\]
Here a Pauli string is a function $p : \mathrm{Fin}\,n \to \{I,X,Y,Z\}$, and its weight
$\mathrm{wt}(p)$ is the number of coordinates $i$ with $p(i) \neq I$; the sum counts,
for each $j \le t$, the $\binom{n}{j}$ choices of a weight-$j$ support together with the
$3^{j}$ assignments of a non-identity Pauli to each chosen coordinate.
\end{theorem}

\begin{proof}
\leanok
Partition by weight $j$ and exact support $S$: each of the $\binom{n}{j}$
supports of size $j$ carries $3^{j}$ strings
(\texttt{card\_pauliStringsExactSupport}, via the \texttt{mkWithSupport}
bijection), and the cells are disjoint.  This volume-of-ball count is used
implicitly by the sphere-packing arguments of the source papers
(Calderbank--Rains--Shor--Sloane cite it as a black box attributed to
Gottesman; Ekert--Macchiavello and Knill--Laflamme use it implicitly) but is
stated in none of them; full note:
\texttt{references/informalized/QuantumHamming.card\_pauliErrorsLe.md}.
\end{proof}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{$n$-qubit Hilbert space}

\begin{definition}[$n$-qubit Hilbert space]
\lean{Hn}
\label{Hn}
\leanok
\statementsource{quant-ph.9608006}{
  pdf-4fa1f624d1c5-p002-b002,
  pdf-4fa1f624d1c5-p003-b002
}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b002}
The $n$-qubit Hilbert space
$\mathcal{H}_n = \ell^2\!\bigl(\{0,1\}^n,\mathbb{C}\bigr)$,
implemented as \texttt{EuclideanSpace ℂ (Fin n → Fin 2)}.
\end{definition}

\begin{lemma}[Complex dimension of the $n$-qubit space]
\lean{finrank_Hn}
\label{finrank_Hn}
\leanok
\uses{Hn}
\difficulty{1}
For every natural number $n$, the $n$-qubit Hilbert space $\mathcal{H}_n$ has complex
dimension $\dim_{\mathbb{C}}(\mathcal{H}_n) = 2^n$.
\end{lemma}

\begin{definition}[Pauli operator]
\lean{pauliOp}
\statementsource{quant-ph.0005008}{
  pdf-1ac7bb3b99cc-p002-b004,
  pdf-1ac7bb3b99cc-p003-b003
}
\label{pauliOp}
\leanok
\statementsource{quant-ph.9608006}{
  pdf-4fa1f624d1c5-p002-b003,
  pdf-4fa1f624d1c5-p003-b002
}
\uses{PauliString, Hn}
For $p \in \mathrm{PauliString}\,n$, the associated Pauli operator
$\hat{p} : \mathcal{H}_n \to \mathcal{H}_n$.
\end{definition}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Knill--Laflamme conditions}

\begin{definition}[Knill--Laflamme condition]
\lean{KnillLaflamme}
\label{KnillLaflamme}
\leanok
\statementsource{quant-ph.9705052}{pdf-e658890bd7ef-p002-b003}
\statementsource{PhysRevA.54.3824}{
  pdf-c11491661f48-p023-b003,
  pdf-c11491661f48-p023-b004
}
\statementsource{PhysRevLett.77.2585}{
  pdf-9634739c3de9-p002-b004,
  pdf-9634739c3de9-p002-b003
}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b005}
\statementsource{PhysRevA.55.900}{pdf-bbdd8e5c3949-p005-b003}
\uses{pauliOp}
A subspace $C \le \mathcal{H}_n$ satisfies the \emph{Knill--Laflamme condition}
for $t$-error correction if for all Pauli strings $E,F$ with $\mathrm{wt}(E),\mathrm{wt}(F) \le t$
there exists $\lambda_{EF}\in\mathbb{C}$ such that
$P_C\,E^\dagger F\,P_C = \lambda_{EF}\,P_C$,
where $P_C$ is the orthogonal projection onto $C$.
\end{definition}

\begin{definition}[Non-degenerate code]
\lean{IsNondegenerate}
\label{IsNondegenerate}
\leanok
\statementsource{quant-ph.9705052}{
  pdf-e658890bd7ef-p002-b005,
  pdf-e658890bd7ef-p005-b005
}
\statementsource{PhysRevA.54.3824}{pdf-c11491661f48-p021-b003}
\statementsource{PhysRevLett.77.2585}{pdf-9634739c3de9-p002-b004}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p004-b008}
\statementsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p003-b002,
  pdf-bbdd8e5c3949-p009-b002
}
\uses{KnillLaflamme}
A code is \emph{non-degenerate} if it satisfies the Knill--Laflamme condition
and additionally $P_C E^\dagger F P_C = 0$ whenever $E \neq F$.
\end{definition}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Error sphere and sphere-packing}

\begin{definition}[Error sphere]
\lean{ErrorSphere}
\label{ErrorSphere}
\leanok
\statementsource{quant-ph.9705052}{pdf-e658890bd7ef-p010-b003}
\statementsource{PhysRevLett.77.2585}{
  pdf-9634739c3de9-p003-b003,
  pdf-9634739c3de9-p003-b001
}
\statementsource{PhysRevA.55.900}{pdf-bbdd8e5c3949-p009-b002}
\uses{pauliOp}
The \emph{error sphere} $\mathrm{ES}(C,t)$ is the subspace
$\bigvee_{\mathrm{wt}(p)\le t} \hat{p}(C)$, i.e.\ the supremum of the
Pauli images of $C$ over all $t$-errors.
\end{definition}

\begin{lemma}[Orthogonality of distinct error subspaces]
\lean{error_subspaces_orthogonal}
\label{error_subspaces_orthogonal}
\leanok
\proofsource{quant-ph.9705052}{
  pdf-e658890bd7ef-p005-b005,
  pdf-e658890bd7ef-p010-b003
}
\proofsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p003-b002,
  pdf-bbdd8e5c3949-p009-b002
}
\statementsource{PhysRevLett.77.2585}{
  pdf-9634739c3de9-p003-b003,
  pdf-9634739c3de9-p002-b004
}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p004-b008}
\statementsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p003-b002,
  pdf-bbdd8e5c3949-p009-b002
}
\uses{IsNondegenerate, pauliOp}
\difficulty{6}
Let $C \le \mathcal{H}_n$ be a code that is non-degenerate for $t$-error correction, and
let $E, F$ be Pauli strings on $n$ qubits with weights $\mathrm{wt}(E) \le t$ and
$\mathrm{wt}(F) \le t$ and $E \neq F$. Then the images $\hat{E}(C)$ and $\hat{F}(C)$ of
$C$ under the corresponding Pauli operators are orthogonal subspaces of $\mathcal{H}_n$;
that is, $\langle \hat{E}\,u,\, \hat{F}\,v\rangle = 0$ for all $u, v \in C$.
\end{lemma}

\begin{lemma}[Dimension of the error sphere of a non-degenerate code]
\lean{error_sphere_dimension}
\label{error_sphere_dimension}
\leanok
\proofsource{quant-ph.9705052}{pdf-e658890bd7ef-p010-b003}
\proofsource{PhysRevA.55.900}{pdf-bbdd8e5c3949-p009-b002}
\proofsource{PhysRevLett.77.2585}{pdf-9634739c3de9-p003-b003}
\statementsource{PhysRevA.55.900}{pdf-bbdd8e5c3949-p009-b002}
\uses{ErrorSphere, error_subspaces_orthogonal, card_pauliErrorsLe}
\difficulty{7}
Let $C \le \mathcal{H}_n$ be a code on $n$ qubits that is non-degenerate for $t$ errors,
meaning it satisfies the Knill--Laflamme condition $P_C E^\dagger F P_C = \lambda_{EF}
P_C$ for all Pauli strings $E,F$ of weight at most $t$ and moreover $P_C E^\dagger F P_C
= 0$ whenever $E \neq F$. Then the error sphere $\mathrm{ES}(C,t) =
\bigvee_{\mathrm{wt}(p)\le t} \hat{p}(C)$ has dimension
\[
\dim_{\mathbb{C}} \mathrm{ES}(C,t) = |\mathcal{E}(n,t)| \cdot \dim_{\mathbb{C}} C,
\]
where $\mathcal{E}(n,t)$ is the set of Pauli strings of weight at most $t$.
\end{lemma}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Quantum Hamming bound}

\begin{theorem}[Quantum Hamming bound for non-degenerate codes]
\lean{quantum_hamming_bound}
\label{quantum_hamming_bound}
\leanok
\proofsource{quant-ph.9705052}{pdf-e658890bd7ef-p010-b003}
\proofsource{PhysRevA.54.3824}{pdf-c11491661f48-p021-b003}
\proofsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p009-b002,
  pdf-bbdd8e5c3949-p009-b003
}
\proofsource{PhysRevLett.77.2585}{pdf-9634739c3de9-p003-b003}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p011-b008}
\statementsource{quant-ph.0508070}{pdf-4bcdc0ee9c86-p009-b002}
\statementsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p009-b002,
  pdf-bbdd8e5c3949-p009-b003
}
\uses{error_sphere_dimension, finrank_Hn}
\difficulty{3}
Let $C \le \mathcal{H}_n$ be a subspace of the $n$-qubit Hilbert space whose complex
dimension is $\dim_{\mathbb{C}} C = 2^k$, and suppose $k \le n$. If $C$ is a
non-degenerate code correcting $t$ errors — meaning it satisfies the Knill–Laflamme
condition $P_C\,E^\dagger F\,P_C = \lambda_{EF}\,P_C$ for all Pauli strings $E,F$ of
weight at most $t$, and moreover $P_C\,E^\dagger F\,P_C = 0$ whenever $E \ne F$, where
$P_C$ is the orthogonal projection onto $C$ — then
\[
\sum_{j=0}^{t} \binom{n}{j}\, 3^{\,j} \;\le\; 2^{\,n-k}.
\]
\end{theorem}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% added by the blueprint pipeline
\section{Additional declarations}

\begin{definition}[Pauli $I$ matrix]
\lean{sigmaI}
\label{sigmaI}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
The $2\times 2$ identity matrix over $\mathbb{C}$, i.e.\ the Pauli operator $I$.
\end{definition}

\begin{definition}[Pauli $X$ matrix]
\lean{sigmaX}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b006}
\label{sigmaX}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
The bit-flip Pauli matrix
$X = \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}$
over $\mathbb{C}$.
\end{definition}

\begin{definition}[Pauli $Y$ matrix]
\lean{sigmaY}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b006}
\label{sigmaY}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
The bit-phase-flip Pauli matrix
$Y = \begin{pmatrix} 0 & -i \\ i & 0 \end{pmatrix}$
over $\mathbb{C}$.
\end{definition}

\begin{definition}[Pauli $Z$ matrix]
\lean{sigmaZ}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b006}
\label{sigmaZ}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
The phase-flip Pauli matrix
$Z = \begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}$
over $\mathbb{C}$.
\end{definition}

\begin{definition}[Matrix of a Pauli basis element]
\lean{PauliBasis.toMatrix}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b006}
\label{PauliBasis.toMatrix}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
\uses{sigmaI, sigmaX, sigmaY, sigmaZ}
Maps each Pauli basis element $\{I,X,Y,Z\}$ to its corresponding $2\times 2$ matrix
$\{\sigma_I,\sigma_X,\sigma_Y,\sigma_Z\}$.
\end{definition}

\begin{definition}[Matrix of a Pauli string]
\lean{pauliMatrix}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p003-b003}
\label{pauliMatrix}
\leanok
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b003}
\uses{PauliBasis.toMatrix, PauliString}
For a Pauli string $p$ on $n$ qubits, the $2^n\times 2^n$ matrix obtained as the
tensor product of the single-qubit Pauli matrices $\sigma_{p(i)}$ over all coordinates~$i$.
\end{definition}

\begin{definition}[Embedding of a non-identity Pauli]
\lean{PauliNZ.toBasis}
\label{PauliNZ.toBasis}
\leanok
Embeds a non-identity Pauli ($X$, $Y$, or $Z$) from the three-element type
$\mathrm{PauliNZ}$ into the full Pauli basis $\{I,X,Y,Z\}$.
\end{definition}

\begin{definition}[Pauli string with prescribed support]
\lean{mkWithSupport}
\label{mkWithSupport}
\leanok
\uses{PauliNZ.toBasis, PauliString}
Given a finset $S \subseteq \mathrm{Fin}\,n$ and an assignment $f : S \to \mathrm{PauliNZ}$,
constructs the Pauli string that equals $f(i).\mathrm{toBasis}$ on each $i \in S$ and equals $I$
elsewhere.
\end{definition}

\begin{lemma}[Support of a Pauli string built from prescribed data]
\lean{support_mkWithSupport}
\label{support_mkWithSupport}
\leanok
\uses{mkWithSupport, support}
\difficulty{1}
Fix $n$, a finset $S \subseteq \mathrm{Fin}\,n$, and an assignment $f : S \to \{X,Y,Z\}$
of a non-identity Pauli to each coordinate in $S$. Let $p$ be the Pauli string of length
$n$ that equals $f(i)$ at each $i \in S$ and equals the identity $I$ at every coordinate
outside $S$. Then the support of $p$ — the set of coordinates where $p$ differs from $I$
— is exactly $S$.
\end{lemma}

\begin{definition}[Pauli strings with exact support]
\lean{pauliStringsExactSupport}
\label{pauliStringsExactSupport}
\leanok
\uses{PauliString, support}
The finset of all Pauli strings on $n$ qubits whose support equals exactly the given
finset $S$.
\end{definition}

\begin{lemma}[Number of Pauli strings with a prescribed support]
\lean{card_pauliStringsExactSupport}
\label{card_pauliStringsExactSupport}
\leanok
\proofsource{quant-ph.9705052}{pdf-e658890bd7ef-p010-b003}
\uses{PauliNZ.toBasis, mkWithSupport, pauliStringsExactSupport, support, support_mkWithSupport}
\difficulty{5}
Let $n$ be a natural number and let $S \subseteq \mathrm{Fin}\,n$ be a set of
coordinates. The number of Pauli strings $p$ of length $n$ whose support
$\mathrm{supp}(p)$ equals exactly $S$ is $3^{|S|}$.
\end{lemma}

\begin{definition}[Quantum code]
\lean{Code}
\label{Code}
\leanok
\statementsource{quant-ph.9705052}{pdf-e658890bd7ef-p002-b002}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p002-b002}
\statementsource{quant-ph.0005008}{pdf-1ac7bb3b99cc-p002-b002}
\statementsource{PhysRevA.55.900}{pdf-bbdd8e5c3949-p004-b002}
\uses{Hn}
An $n$-qubit quantum code is a subspace of the $n$-qubit Hilbert space $\mathcal{H}_n$.
\end{definition}

\begin{definition}[Adjoint of a Pauli operator]
\lean{pauliOpAdjoint}
\label{pauliOpAdjoint}
\leanok
\uses{Hn, PauliString, pauliMatrix}
The adjoint (Hermitian conjugate) $\hat{p}^\dagger : \mathcal{H}_n \to \mathcal{H}_n$ of the
Pauli operator associated with a Pauli string $p$.
\end{definition}

\begin{definition}[Projection onto a code]
\lean{codeProj}
\label{codeProj}
\leanok
\uses{Hn}
The orthogonal projection of $\mathcal{H}_n$ onto a subspace $C$, viewed as an endomorphism
$P_C : \mathcal{H}_n \to \mathcal{H}_n$.
\end{definition}

\begin{lemma}[Action of the code projection]
\lean{codeProj_apply}
\label{codeProj_apply}
\leanok
\uses{Hn, codeProj}
\difficulty{1}
Let $C$ be a closed subspace of the $n$-qubit Hilbert space $\mathcal{H}_n$, and let
$P_C : \mathcal{H}_n \to \mathcal{H}_n$ be the associated orthogonal projection onto
$C$. Then for every $x \in \mathcal{H}_n$, the value $P_C\,x$ is exactly the orthogonal
projection of $x$ onto $C$.
\end{lemma}

\begin{lemma}[Image of the code projection lies in the code]
\lean{codeProj_mem}
\label{codeProj_mem}
\leanok
\uses{Hn, codeProj, codeProj_apply}
\difficulty{2}
Let $C$ be a subspace of the $n$-qubit Hilbert space $\mathcal{H}_n$, and let $P_C :
\mathcal{H}_n \to \mathcal{H}_n$ be the orthogonal projection onto $C$. Then for every
$x \in \mathcal{H}_n$, the projected vector $P_C\,x$ belongs to $C$.
\end{lemma}

\begin{lemma}[Projection onto a code fixes its own vectors]
\lean{codeProj_eq_self_of_mem}
\label{codeProj_eq_self_of_mem}
\leanok
\uses{Hn, codeProj}
\difficulty{2}
Let $C$ be a subspace of the $n$-qubit Hilbert space $\mathcal{H}_n$, and let $P_C :
\mathcal{H}_n \to \mathcal{H}_n$ be the orthogonal projection onto $C$. If $x \in C$,
then $P_C x = x$.
\end{lemma}

\begin{lemma}[Idempotence of the code projection]
\lean{codeProj_idempotent}
\label{codeProj_idempotent}
\leanok
\uses{Hn, codeProj, codeProj_eq_self_of_mem, codeProj_mem}
\difficulty{2}
Let $C$ be a closed subspace of the $n$-qubit Hilbert space $\mathcal{H}_n =
\ell^2(\{0,1\}^n, \bbc)$, and let $P_C : \mathcal{H}_n \to \mathcal{H}_n$ be the
orthogonal projection onto $C$. Then $P_C$ is idempotent: for every $x \in
\mathcal{H}_n$, \[ P_C(P_C\, x) = P_C\, x. \]
\end{lemma}

\begin{theorem}[Quantum Hamming bound for non-degenerate codes]
\lean{quantum_hamming_bound_raw}
\label{quantum_hamming_bound_raw}
\leanok
\proofsource{quant-ph.9705052}{pdf-e658890bd7ef-p010-b003}
\proofsource{PhysRevA.54.3824}{pdf-c11491661f48-p021-b003}
\proofsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p009-b002,
  pdf-bbdd8e5c3949-p009-b003
}
\proofsource{PhysRevLett.77.2585}{pdf-9634739c3de9-p003-b003}
\statementsource{quant-ph.9608006}{pdf-4fa1f624d1c5-p011-b008}
\statementsource{quant-ph.0508070}{pdf-4bcdc0ee9c86-p009-b002}
\statementsource{PhysRevA.55.900}{
  pdf-bbdd8e5c3949-p009-b002,
  pdf-bbdd8e5c3949-p009-b003
}
\uses{ErrorSphere, Hn, IsNondegenerate, card_pauliErrorsLe, error_sphere_dimension, finrank_Hn}
\difficulty{3}
Let $C \le \mathcal{H}_n$ be a subspace of the $n$-qubit Hilbert space that is a
non-degenerate quantum code correcting $t$ errors, and suppose its complex dimension is
$\dim_{\mathbb{C}} C = 2^{k}$ for some natural number $k$. Then
\[
\left(\sum_{j=0}^{t}\binom{n}{j}\,3^{j}\right)\cdot 2^{k} \;\le\; 2^{n}.
\]
\end{theorem}

\begin{lemma}[Non-identity Paulis embed away from the identity]
\lean{PauliNZ.toBasis_ne_I}
\label{PauliNZ.toBasis_ne_I}
\leanok
\uses{PauliNZ.toBasis}
\difficulty{1}
Write $\{X, Y, Z\}$ for the three non-identity Pauli labels, embedded in the natural way
into the full Pauli basis $\{I, X, Y, Z\}$. Then the image of every such label differs
from the identity element $I$.
\end{lemma}

\begin{definition}[Non-identity Pauli labels]
\lean{PauliNZ}
\label{PauliNZ}
\leanok
The three-element type $\{X, Y, Z\}$ of labels for the non-identity single-qubit Pauli
operators. It serves as the index set in counting arguments that enumerate, for a fixed
support, the $3^{|S|}$ assignments of a non-identity Pauli to each supported coordinate.
\end{definition}
