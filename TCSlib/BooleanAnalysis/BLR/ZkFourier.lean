...
lemma zkDot_sub (s t x : ZkVec k n) :
    zkDot (s - t) x = zkDot s x - zkDot t x := by
      have h_expand : ∀ i : Fin n, (s - t) i * x i = s i * x i - t i * x i := by
        exact fun i => by rw [ Pi.sub_apply, sub_mul ] ;
      unfold zkDot; aesop;
...
lemma char_s_ne_zero (s : ZkVec k n) (x : ZkVec k n) :
    char_s s x ≠ 0 := by
  intro h; have := norm_char_s s x; rw [h, norm_zero] at this; exact one_ne_zero this.symm
...
lemma char_s_mul (s t : ZkVec k n) (x : ZkVec k n) :
    char_s s x * char_s t x = char_s (s + t) x := by
      ...
      · convert toOmega_add ( zkDot s x ) ( zkDot t x ) using 1;
        have h_dot : zkDot (s + t) x = ∑ i, (s i + t i) * x i := by
          rfl;
        convert congr_arg ( fun z => toOmega z ) h_dot using 1;
...
lemma expectation_char_nontrivial {s : ZkVec k n} (hs : s ≠ 0) :
    expectation (char_s s) = 0 := by
      ...
      have h_fubini : ∑ x : ZkVec k n, toOmega (zkDot s x) = ∏ i : Fin n, ∑ x_i : ZMod k, toOmega (s i * x_i) := by
        rw [ Finset.prod_sum ];
      ...
      unfold expectation char_s; simp_all +decide  ;
...
lemma fourier_expansion (f : ZkFun k n) (x : ZkVec k n) :
    f x = ∑ s : ZkVec k n, fourier_coeff f s * char_s s x := by
      have h_dual_orthogonality : ∀ (x y : ZkVec k n), ∑ s : ZkVec k n, char_s s (x - y) = if x = y then (k : ℂ) ^ n else 0 := by
        intro x y; split_ifs with h; simp_all +decide ;
      ...
      have h_exchange_order : ∑ s : ZkVec k n, (∑ y : ZkVec k n, f y * starRingEnd ℂ (char_s s y) / (k : ℂ) ^ n) * char_s s x = ∑ y : ZkVec k n, f y * (∑ s : ZkVec k n, char_s s (x - y)) / (k : ℂ) ^ n := by
      ...
      simp_all +decide [ fourier_coeff, inner_product ];
      unfold expectation; simp_all +decide [ Finset.sum_div _ _ _ ] ;
...
lemma parseval_identity (f : ZkFun k n) :
    ∑ s : ZkVec k n, ‖fourier_coeff f s‖ ^ 2 = L2_norm_sq f := by
      ...
      have h_L2_norm_sq : L2_norm_sq f = (∑ x : ZkVec k n, f x * starRingEnd ℂ (f x)) / (k : ℂ) ^ n := by
      ...
      have h_fourier_coeff : ∑ s : ZkVec k n, ‖fourier_coeff f s‖ ^ 2 = (∑ s : ZkVec k n, fourier_coeff f s * starRingEnd ℂ (fourier_coeff f s)) := by
      ...
      have h_substitute : ∑ s : ZkVec k n, fourier_coeff f s * starRingEnd ℂ (fourier_coeff f s) = ∑ x : ZkVec k n, (∑ s : ZkVec k n, fourier_coeff f s * char_s s x) * starRingEnd ℂ (f x) / (k : ℂ) ^ n := by
        have h_fourier_coeff_def : ∀ s : ZkVec k n, fourier_coeff f s = (∑ x : ZkVec k n, f x * starRingEnd ℂ (char_s s x)) / (k : ℂ) ^ n := by
          exact fun s => Complex.ext rfl rfl;
        simp +decide only [h_fourier_coeff_def, sum_div];
      ...
      have h_inner_sum : ∀ x : ZkVec k n, ∑ s : ZkVec k n, fourier_coeff f s * char_s s x = f x := by
        exact fun x => Eq.symm (fourier_expansion f x);
      simp_all +decide [ ← Finset.sum_div _ _ _ ];
      exact_mod_cast h_fourier_coeff.trans h_L2_norm_sq.symm
