...
-- f linear ↔ ∃s, f(x) = s·x
private lemma linear_iff_character_aux_h_ind {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (h : is_linear f) (i : Fin n) (m : Nat) : f (Pi.single i (m : ZMod k)) = m * f (Pi.single i 1) := by
  induction m <;> simp_all +decide [ add_mul, Pi.single_add ] ;
  · simpa using h 0 0;
    ...
    is_linear f ↔ ∃ s : ZkVec k n, ∀ x, f x = zkDot s x := by
      ...
      · use fun i => f ( Pi.single i 1 );
        ...
        · -- Scalar step: f(m · e_i) = m · f(e_i) by induction on m.
          rename_i i m;
          convert (linear_iff_character_aux_h_ind f h i) ( m.val ) using 1 <;> simp +decide [ zkDot ];
...
private lemma re_toOmega_le_re_rootOfUnity_aux_h_a_val {k : Nat} [inst : NeZero k] (hk : 2 ≤ k) (a : ZMod k) (ha : a ≠ 0) : 1 ≤ a.val ∧ a.val ≤ k - 1 := by
  exact ⟨ Nat.pos_of_ne_zero ( by simpa [ ZMod.val_eq_zero ] using ha ), Nat.le_pred_of_lt ( ZMod.val_lt a ) ⟩;

private lemma re_toOmega_le_re_rootOfUnity_aux_h_angle {k : Nat} [inst : NeZero k] (hk : 2 ≤ k) (a : ZMod k) (ha : a ≠ 0) (h_a_val : 1 ≤ a.val ∧ a.val ≤ k - 1) : 2 * Real.pi / k ≤ 2 * Real.pi * a.val / k ∧ 2 * Real.pi * a.val / k ≤ 2 * Real.pi - 2 * Real.pi / k := by
  field_simp;
  exact ⟨ mod_cast h_a_val.1, le_tsub_of_add_le_right <| mod_cast h_a_val.2.trans_lt <| Nat.pred_lt <| ne_bot_of_gt hk ⟩;

private lemma re_toOmega_le_re_rootOfUnity_aux_h_cos_decreasing {k : Nat} [inst : NeZero k] (hk : 2 ≤ k) (a : ZMod k) (ha : a ≠ 0) (h_a_val : 1 ≤ a.val ∧ a.val ≤ k - 1) (h_angle : 2 * Real.pi / ↑k ≤ 2 * Real.pi * ↑a.val / ↑k ∧ 2 * Real.pi * ↑a.val / ↑k ≤ 2 * Real.pi - 2 * Real.pi / ↑k) : Real.cos (2 * Real.pi * a.val / k) ≤ Real.cos (2 * Real.pi / k) := by
...
lemma re_toOmega_le_re_rootOfUnity {k : ℕ} [NeZero k] (hk : 2 ≤ k) (a : ZMod k) (ha : a ≠ 0) :
  ...
  convert (re_toOmega_le_re_rootOfUnity_aux_h_cos_decreasing hk a ha (re_toOmega_le_re_rootOfUnity_aux_h_a_val hk a ha) (re_toOmega_le_re_rootOfUnity_aux_h_angle hk a ha (re_toOmega_le_re_rootOfUnity_aux_h_a_val hk a ha))) using 1 ; unfold toOmega ; norm_num [ Complex.exp_re, Complex.exp_im, Complex.cos ] ; ring_nf;
...
-/
private lemma re_fourier_coeff_upper_bound_aux_h_ineq {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (s x : ZkVec k n) (hfx : ¬f x = linear_character s x) : (toOmega (f x - zkDot s x)).re ≤ Real.cos (2 * Real.pi / k) := by
  apply re_toOmega_le_re_rootOfUnity hk;
  exact sub_ne_zero_of_ne hfx;

private lemma re_fourier_coeff_upper_bound_aux_h_ineq__dup3 {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (s x : ZkVec k n) (hfx : ¬f x = linear_character s x) (h_ineq : (toOmega (f x - zkDot s x)).re ≤ Real.cos (2 * Real.pi / ↑k)) : toOmega (f x - zkDot s x) = toOmega (f x) * toOmega (-zkDot s x) := by
  convert toOmega_add ( f x ) ( -zkDot s x ) using 1 ; ring_nf;

private lemma re_fourier_coeff_upper_bound_aux_h_ineq__dup2 {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (s x : ZkVec k n) (hfx : ¬f x = linear_character s x) (h_ineq : (toOmega (f x - zkDot s x)).re ≤ Real.cos (2 * Real.pi / ↑k)) : toOmega (f x - zkDot s x) = toOmega (f x) * (starRingEnd ℂ) (toOmega (zkDot s x)) := by
  rw [ (re_fourier_coeff_upper_bound_aux_h_ineq__dup3 hk f s x hfx h_ineq), ← toOmega_neg ];

private lemma re_fourier_coeff_upper_bound_aux_h_bound_tail_h_ineq_tail_h_ineq__dup2 {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (s x : ZkVec k n) (hfx : ¬f x = linear_character s x) (h_ineq : (toOmega (f x - zkDot s x)).re ≤ Real.cos (2 * Real.pi / ↑k)) (h_ineq__dup2 : toOmega (f x - zkDot s x) = toOmega (f x) * (starRingEnd ℂ) (toOmega (zkDot s x))) : (toOmega (f x) * (starRingEnd ℂ) (toOmega (zkDot s x))).re ≤ 1 - (1 - Real.cos (2 * Real.pi / ↑k)) * if f x = linear_character s x then 0 else 1 := by
  aesop;



private lemma re_fourier_coeff_upper_bound_aux_h_bound_tail_h_ineq {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (s x : ZkVec k n) (hfx : ¬f x = linear_character s x) (h_ineq : (toOmega (f x - zkDot s x)).re ≤ Real.cos (2 * Real.pi / ↑k)) : (toOmega (f x) * (starRingEnd ℂ) (toOmega (zkDot s x))).re ≤ 1 - (1 - Real.cos (2 * Real.pi / ↑k)) * if f x = linear_character s x then 0 else 1 := by
  -- Rewrite ω_k^{f(x) − s·x} = ω_k^{f(x)} · conj(ω_k^{s·x}).
  exact (re_fourier_coeff_upper_bound_aux_h_bound_tail_h_ineq_tail_h_ineq__dup2 hk f s x hfx h_ineq ((re_fourier_coeff_upper_bound_aux_h_ineq__dup2 hk f s x hfx h_ineq)))



private lemma re_fourier_coeff_upper_bound_aux_h_bound {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (s x : ZkVec k n) : (toOmega (f x) * (starRingEnd ℂ) (toOmega (zkDot s x))).re ≤ 1 - (1 - Real.cos (2 * Real.pi / k)) * (if f x = linear_character s x then 0 else 1) := by
  ...
  · -- Disagreement case: ω_k^{f(x) − s·x} has Re ≤ cos(2π/k).
    exact (re_fourier_coeff_upper_bound_aux_h_bound_tail_h_ineq hk f s x hfx ((re_fourier_coeff_upper_bound_aux_h_ineq hk f s x hfx)))
...
lemma re_fourier_coeff_upper_bound
  ...
  convert div_le_div_of_nonneg_right ( Finset.sum_le_sum fun x _ => (re_fourier_coeff_upper_bound_aux_h_bound hk f s) x ) ( by positivity : ( 0 : ℝ ) ≤ k ^ n ) using 1 <;> norm_num [ Finset.sum_ite ] ; ring_nf;
...
private lemma indicator_eq_char_sum_re_aux_h {k : Nat} [inst : NeZero k] (a : ZMod k) : ∑ j, toOmega (j * a) = if a = 0 then ↑k else 0 :=
  geom_sum_toOmega_dual a

lemma indicator_eq_char_sum_re (a : ZMod k) :
    (if a = 0 then (1 : ℝ) else 0) =
    (1 / (k : ℝ)) * (∑ j : ZMod k, toOmega (j * a)).re := by
  let h : ∑ j, toOmega (j * a) = if a = 0 then ↑k else 0 := (indicator_eq_char_sum_re_aux_h a)
  split_ifs at * <;> simp_all +decide [NeZero.ne, h]
...
-/
private lemma triple_product_fourier_aux_h_fourier_expansion {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) (x y : ZkVec k n) : F (x + y) = ∑ s : ZkVec k n, fourier_coeff F s * char_s s (x + y) := by
  exact fourier_expansion F ( x + y );

private lemma triple_product_fourier_aux_h_exchange_sum {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) (h_fourier_expansion : ∀ x y : ZkVec k n, F (x + y) = ∑ s : ZkVec k n, fourier_coeff F s * char_s s (x + y)) : ∑ x : ZkVec k n, ∑ y : ZkVec k n, F (x + y) * starRingEnd ℂ (F x) * starRingEnd ℂ (F y) = ∑ s : ZkVec k n, fourier_coeff F s * ∑ x : ZkVec k n, starRingEnd ℂ (F x) * char_s s x * ∑ y : ZkVec k n, starRingEnd ℂ (F y) * char_s s y := by
...
private lemma triple_product_fourier_aux_h_inner_sum__dup2 {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) (h_fourier_expansion : ∀ x y : ZkVec k n, F (x + y) = ∑ s : ZkVec k n, fourier_coeff F s * char_s s (x + y)) (h_exchange_sum : ∑ x : ZkVec k n, ∑ y : ZkVec k n, F (x + y) * starRingEnd ℂ (F x) * starRingEnd ℂ (F y) = ∑ s : ZkVec k n, fourier_coeff F s * ∑ x : ZkVec k n, starRingEnd ℂ (F x) * char_s s x * ∑ y : ZkVec k n, starRingEnd ℂ (F y) * char_s s y) (s : ZkVec k n) : ∑ x : ZkVec k n, starRingEnd ℂ (F x) * char_s s x = starRingEnd ℂ (∑ x : ZkVec k n, F x * starRingEnd ℂ (char_s s x)) := by
  simp +decide [ mul_comm];

private lemma triple_product_fourier_aux_h_inner_sum {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) (h_fourier_expansion : ∀ x y : ZkVec k n, F (x + y) = ∑ s : ZkVec k n, fourier_coeff F s * char_s s (x + y)) (h_exchange_sum : ∑ x : ZkVec k n, ∑ y : ZkVec k n, F (x + y) * starRingEnd ℂ (F x) * starRingEnd ℂ (F y) = ∑ s : ZkVec k n, fourier_coeff F s * ∑ x : ZkVec k n, starRingEnd ℂ (F x) * char_s s x * ∑ y : ZkVec k n, starRingEnd ℂ (F y) * char_s s y) (s : ZkVec k n) : ∑ x : ZkVec k n, starRingEnd ℂ (F x) * char_s s x = starRingEnd ℂ (fourier_coeff F s) * (k : ℂ) ^ n := by
...
private lemma triple_product_fourier_aux_h_fourier_tail_h_exchange_sum_tail_h_inner_sum {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) (h_exchange_sum : ∑ x, ∑ y, F (x + y) * (starRingEnd ℂ) (F x) * (starRingEnd ℂ) (F y) = ∑ s, fourier_coeff F s * ∑ x, (starRingEnd ℂ) (F x) * char_s s x * ∑ y, (starRingEnd ℂ) (F y) * char_s s y) (h_inner_sum : ∀ (s : ZkVec k n), ∑ x, (starRingEnd ℂ) (F x) * char_s s x = (starRingEnd ℂ) (fourier_coeff F s) * ↑k ^ n) : (expectation fun x => expectation fun y => F (x + y) * (starRingEnd ℂ) (F x) * (starRingEnd ℂ) (F y)) = ∑ s, ↑‖fourier_coeff F s‖ ^ 2 * (starRingEnd ℂ) (fourier_coeff F s) := by
...
private lemma triple_product_fourier_aux_h_fourier_tail_h_exchange_sum {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) (h_exchange_sum : ∑ x, ∑ y, F (x + y) * (starRingEnd ℂ) (F x) * (starRingEnd ℂ) (F y) = ∑ s, fourier_coeff F s * ∑ x, (starRingEnd ℂ) (F x) * char_s s x * ∑ y, (starRingEnd ℂ) (F y) * char_s s y) : (expectation fun x => expectation fun y => F (x + y) * (starRingEnd ℂ) (F x) * (starRingEnd ℂ) (F y)) = ∑ s, ↑‖fourier_coeff F s‖ ^ 2 * (starRingEnd ℂ) (fourier_coeff F s) := by
  ...
  exact (triple_product_fourier_aux_h_fourier_tail_h_exchange_sum_tail_h_inner_sum F h_exchange_sum ((triple_product_fourier_aux_h_inner_sum F ((triple_product_fourier_aux_h_fourier_expansion F)) h_exchange_sum)))
...
private lemma triple_product_fourier_aux_h_fourier {k : Nat} [inst : NeZero k] {n : Nat} (F : ZkFun k n) : expectation (fun x => expectation (fun y => F (x + y) * starRingEnd ℂ (F x) * starRingEnd ℂ (F y))) = ∑ s : ZkVec k n, ‖fourier_coeff F s‖ ^ 2 * starRingEnd ℂ (fourier_coeff F s) := by
  ...
  exact (triple_product_fourier_aux_h_fourier_tail_h_exchange_sum F ((triple_product_fourier_aux_h_exchange_sum F ((triple_product_fourier_aux_h_fourier_expansion F)))))
...
lemma triple_product_fourier (j : ZMod k) (f : ZkVec k n → ZMod k) :
  ...
  exact (triple_product_fourier_aux_h_fourier) _

/-
BLR acceptance probability equals `(1/k) ∑_j ∑_s |f̂_j(s)|² Re[f̂_j(s)]`.
-/
private lemma BLR_accept_prob_eq_fourier_sum_aux_h_anon_1 {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (x y : ZkVec k n) : (if f (x + y) - f x - f y = 0 then 1 else 0) = 1 / ↑k * (∑ j, toOmega (j * (f (x + y) - f x - f y))).re :=
  indicator_eq_char_sum_re ( f ( x + y ) - f x - f y )

private lemma BLR_accept_prob_eq_fourier_sum_aux_h_indicator_tail_h_anon_1 {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (x y : ZkVec k n) (h_anon_1 : (if f (x + y) - f x - f y = 0 then 1 else 0) = 1 / ↑k * (∑ j, toOmega (j * (f (x + y) - f x - f y))).re) : (if f (x + y) = f x + f y then 1 else 0) = 1 / ↑k * ∑ j, (toOmega (j * (f (x + y) - f x - f y))).re := by
  simp_all +decide [ sub_eq_iff_eq_add' ] ;


private lemma BLR_accept_prob_eq_fourier_sum_aux_h_indicator {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (x y : ZkVec k n) : (if f (x + y) = f x + f y then (1 : ℝ) else 0) = (1 / (k : ℝ)) * (∑ j : ZMod k, (toOmega (j * (f (x + y) - f x - f y))).re) := by
  exact (BLR_accept_prob_eq_fourier_sum_aux_h_indicator_tail_h_anon_1 f x y ((BLR_accept_prob_eq_fourier_sum_aux_h_anon_1 f x y)))


private lemma BLR_accept_prob_eq_fourier_sum_aux_h_triple_product__dup2 {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (h_indicator : ∀ x y : ZkVec k n, (if f (x + y) = f x + f y then (1 : ℝ) else 0) = (1 / (k : ℝ)) * (∑ j : ZMod k, (toOmega (j * (f (x + y) - f x - f y))).re)) (j : ZMod k) : ∑ x : ZkVec k n, ∑ y : ZkVec k n, (toOmega (j * (f (x + y) - f x - f y))) = (k : ℂ) ^ (2 * n) * (∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * starRingEnd ℂ (fourier_coeff (lift_omega_j j f) s)) := by
...
private lemma BLR_accept_prob_eq_fourier_sum_aux_h_triple_product {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (h_indicator : ∀ x y : ZkVec k n, (if f (x + y) = f x + f y then (1 : ℝ) else 0) = (1 / (k : ℝ)) * (∑ j : ZMod k, (toOmega (j * (f (x + y) - f x - f y))).re)) (j : ZMod k) : ∑ x : ZkVec k n, ∑ y : ZkVec k n, (toOmega (j * (f (x + y) - f x - f y))).re = (k : ℝ) ^ (2 * n) * (∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re) := by
  -- Work in ℂ first, then take real parts.
  -- Take real parts on both sides.
  convert congr_arg Complex.re (BLR_accept_prob_eq_fourier_sum_aux_h_triple_product__dup2 f h_indicator j) using 1;
  · simp +decide ;
...
lemma BLR_accept_prob_eq_fourier_sum
  ...
  unfold BLR_accept_prob;
  simp +decide only [(BLR_accept_prob_eq_fourier_sum_aux_h_indicator f)];
  simp +decide [ ← Finset.mul_sum _ _ _ ];
  rw [ Finset.sum_comm ];
  rw [ Finset.sum_comm, Finset.sum_congr rfl fun _ _ => Finset.sum_comm ];
  rw [ Finset.sum_comm, Finset.sum_congr rfl fun _ _ => (BLR_accept_prob_eq_fourier_sum_aux_h_triple_product f (BLR_accept_prob_eq_fourier_sum_aux_h_indicator f)) _ ] ; ring_nf;
  simp +decide [ Finset.mul_sum _ _ _, mul_assoc, mul_left_comm, NeZero.ne ]

/-
The `j = 0` contribution to the BLR Fourier sum equals `1`.
-/
private lemma lift_omega_j_zero_contribution_aux_h_exp__dup2 {k : Nat} [inst : NeZero k] {n : Nat} (s : ZkVec k n) (hs_nonzero : s ≠ 0) : expectation (fun x => char_s (-s) x) = 0 := by
  exact expectation_char_nontrivial ( neg_ne_zero.mpr hs_nonzero );

private lemma lift_omega_j_zero_contribution_aux_h_exp {k : Nat} [inst : NeZero k] {n : Nat} (s : ZkVec k n) (hs_nonzero : s ≠ 0) : expectation (fun x => starRingEnd ℂ (char_s s x)) = 0 := by
  simp_rw [ZkFourier.char_s_conj]; exact (lift_omega_j_zero_contribution_aux_h_exp__dup2 s hs_nonzero);

private lemma lift_omega_j_zero_contribution_aux_h_nonzero_tail_h_exp {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (s : ZkVec k n) (hs_nonzero : s ≠ 0) (h_exp : (expectation fun x => (starRingEnd ℂ) (char_s s x)) = 0) : (expectation fun x => lift_omega_j 0 f x * (starRingEnd ℂ) (char_s s x)) = 0 := by
  unfold lift_omega_j at *; simp_all +decide [ expectation ] ;


private lemma lift_omega_j_zero_contribution_aux_h_nonzero {k : Nat} [inst : NeZero k] {n : Nat} (f : ZkVec k n → ZMod k) (s : ZkVec k n) : s ≠ 0 → expectation (fun x => lift_omega_j 0 f x * starRingEnd ℂ (char_s s x)) = 0 := by
  intro hs_nonzero
  -- The expectation reduces to E[conj(χ_s)] = E[χ_{-s}] = 0 since s ≠ 0.
  exact (lift_omega_j_zero_contribution_aux_h_nonzero_tail_h_exp f s hs_nonzero ((lift_omega_j_zero_contribution_aux_h_exp s hs_nonzero)))
...
lemma lift_omega_j_zero_contribution
  ...
  let h_nonzero : ∀ s : ZkVec k n, s ≠ 0 → expectation (fun x => lift_omega_j 0 f x * starRingEnd ℂ (char_s s x)) = 0 := (lift_omega_j_zero_contribution_aux_h_nonzero f)
  -- Only the s = 0 term survives; it contributes |1|² · Re(1) = 1.
  rw [ Finset.sum_eq_single 0 ] <;> simp_all +decide [h_nonzero];
...
lemma weighted_fourier_sum_le_one
  ...
  · have h_anon_1 := parseval_lift_omega_j j f;
    exact le_trans ( Real.le_sqrt_of_sq_le ( Finset.single_le_sum ( fun s _ => sq_nonneg ( ‖fourier_coeff ( lift_omega_j j f ) s‖ ) ) ( Finset.mem_univ i ) ) ) ( by norm_num [ h_anon_1 ] );
...
private lemma cube_sum_bound_by_max_aux_hs_nonneg {p : Nat} [inst : Fact (Nat.Prime p)] {n : Nat} (f : ZkVec p n → ZMod p) (A : ℝ) (hA : ∀ (s : ZkVec p n), ‖fourier_coeff (lift_omega f) s‖ ≤ A) (s : ZkVec p n) : 0 ≤ ‖fourier_coeff (lift_omega f) s‖ :=
  norm_nonneg _

private lemma cube_sum_bound_by_max_aux_hsq_nonneg {p : Nat} [inst : Fact (Nat.Prime p)] {n : Nat} (f : ZkVec p n → ZMod p) (A : ℝ) (hA : ∀ (s : ZkVec p n), ‖fourier_coeff (lift_omega f) s‖ ≤ A) (s : ZkVec p n) (hs_nonneg : 0 ≤ ‖fourier_coeff (lift_omega f) s‖) : 0 ≤ ‖fourier_coeff (lift_omega f) s‖ ^ 2 := by
  positivity

private lemma cube_sum_bound_by_max_aux_hs_le {p : Nat} [inst : Fact (Nat.Prime p)] {n : Nat} (f : ZkVec p n → ZMod p) (A : ℝ) (hA : ∀ (s : ZkVec p n), ‖fourier_coeff (lift_omega f) s‖ ≤ A) (s : ZkVec p n) (hs_nonneg : 0 ≤ ‖fourier_coeff (lift_omega f) s‖) (hsq_nonneg : 0 ≤ ‖fourier_coeff (lift_omega f) s‖ ^ 2) : ‖fourier_coeff (lift_omega f) s‖ ≤ A :=
  hA s

private lemma cube_sum_bound_by_max_aux_hpoint {p : Nat} [inst : Fact (Nat.Prime p)] {n : Nat} (f : ZkVec p n → ZMod p) (A : ℝ) (hA : ∀ (s : ZkVec p n), ‖fourier_coeff (lift_omega f) s‖ ≤ A) (s : ZkVec p n) : ‖fourier_coeff (lift_omega f) s‖ ^ 3 ≤ A * ‖fourier_coeff (lift_omega f) s‖ ^ 2 := by
  let hs_le : ‖fourier_coeff (lift_omega f) s‖ ≤ A := (cube_sum_bound_by_max_aux_hs_le f A hA s (cube_sum_bound_by_max_aux_hs_nonneg f A hA s) (cube_sum_bound_by_max_aux_hsq_nonneg f A hA s (cube_sum_bound_by_max_aux_hs_nonneg f A hA s)))
  calc
    ...
    ∑ s : ZkVec p n,
      ...
      ∑ s : ZkVec p n,
        A * ‖fourier_coeff (lift_omega f) s‖ ^ 2 := by
          exact Finset.sum_le_sum (fun s _ => (cube_sum_bound_by_max_aux_hpoint f A hA) s)
    _ =
      A *
      ∑ s : ZkVec p n,
        ‖fourier_coeff (lift_omega f) s‖ ^ 2 := by
...
private lemma parseval_lift_omega_aux_hnorm {p : Nat} [inst : Fact (Nat.Prime p)] {n : Nat} (f : ZkVec p n → ZMod p) (x : ZkVec p n) : ‖lift_omega f x‖ ^ 2 = (1 : ℝ) := by
  unfold lift_omega
  rw [norm_toOmega]
  norm_num
...
lemma parseval_lift_omega
  ...
  simp [(parseval_lift_omega_aux_hnorm f), card_ZkVec]
...
private lemma re_fourier_coeff_lift_omega_j_bound_aux_hg_far {p : Nat} [inst : Fact (Nat.Prime p)] {n : Nat} (f : ZkVec p n → ZMod p) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) (j : ZMod p) (hj : j ≠ 0) : epsilon_far_from_linear_normalized (fun x => j * f x) ε := by
...
lemma re_fourier_coeff_lift_omega_j_bound
  ...
  convert re_epsilon_far_bounds_fourier (Nat.Prime.two_le Fact.out) g ε (re_fourier_coeff_lift_omega_j_bound_aux_hg_far f ε hfar j hj) s using 1
...
private lemma re_fourier_coeff_lift_omega_j_unit_bound_aux_hg {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) (j : (ZMod k)ˣ) : epsilon_far_from_linear_normalized (fun x => ↑j * f x) ε :=
  unit_mul_epsilon_far f ε hfar j
...
lemma re_fourier_coeff_lift_omega_j_unit_bound
  ...
  convert re_epsilon_far_bounds_fourier hk g ε (re_fourier_coeff_lift_omega_j_unit_bound_aux_hg hk f ε hfar j) s using 1

-- For a unit `j`, if `f` is ε-far from linear, the weighted Fourier sum is at most `1 − (1 − cos(2π/k))ε`.
private lemma weighted_sum_unit_bound_aux_h_coeff_unit_bound {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) (j : (ZMod k)ˣ) (s : ZkVec k n) : (fourier_coeff (lift_omega_j (↑j) f) s).re ≤ 1 - (1 - Real.cos (2 * Real.pi / k)) * ε := by
  exact re_fourier_coeff_lift_omega_j_unit_bound hk f ε hfar j s
...
lemma weighted_sum_unit_bound
  ...
  refine' le_trans ( Finset.sum_le_sum fun s _ => mul_le_mul_of_nonneg_left ( (weighted_sum_unit_bound_aux_h_coeff_unit_bound hk f ε hfar j) s ) ( sq_nonneg _ ) ) _;
...
-/
private lemma BLR_soundness_general_aux_h_split_sum {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) : ∑ j : ZMod k, ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re = ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re + ∑ j ∈ Finset.filter (fun j => ¬IsUnit j) (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re := by
  rw [ Finset.sum_filter_add_sum_filter_not ];

private lemma BLR_soundness_general_aux_h_units_bound {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) (h_split_sum : ∑ j : ZMod k, ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re = ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re + ∑ j ∈ Finset.filter (fun j => ¬IsUnit j) (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re) (j : (ZMod k)ˣ) : ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j (↑j) f) s‖ ^ 2 * (fourier_coeff (lift_omega_j (↑j) f) s).re ≤ 1 - (1 - Real.cos (2 * Real.pi / k)) * ε := by
  exact weighted_sum_unit_bound hk f ε hfar j;

private lemma BLR_soundness_general_aux_h_unit_bound {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) (h_split_sum : ∑ j : ZMod k, ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re = ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re + ∑ j ∈ Finset.filter (fun j => ¬IsUnit j) (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re) : ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re ≤ (k.totient : ℝ) * (1 - (1 - Real.cos (2 * Real.pi / k)) * ε) := by
  -- Reindex the sum over units to a sum over (ZMod k)ˣ.
  convert Finset.sum_le_sum fun j ( hj : j ∈ Finset.univ ) => (BLR_soundness_general_aux_h_units_bound hk f ε hfar h_split_sum) j;
...
private lemma BLR_soundness_general_aux_h_nonunit_bound {k : Nat} [inst : NeZero k] {n : Nat} (hk : 2 ≤ k) (f : ZkVec k n → ZMod k) (ε : ℝ) (hfar : epsilon_far_from_linear_normalized f ε) (h_split_sum : ∑ j : ZMod k, ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re = ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re + ∑ j ∈ Finset.filter (fun j => ¬IsUnit j) (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re) (h_unit_bound : ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re ≤ (k.totient : ℝ) * (1 - (1 - Real.cos (2 * Real.pi / k)) * ε)) : ∑ j ∈ Finset.filter (fun j => ¬IsUnit j) (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re ≤ (k - k.totient : ℝ) := by
...
lemma BLR_soundness_general
  ...
  · -- Split the sum over j into units and non-units.
    ...
    let h_unit_bound : ∑ j ∈ Finset.filter IsUnit (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re ≤ (k.totient : ℝ) * (1 - (1 - Real.cos (2 * Real.pi / k)) * ε) := (BLR_soundness_general_aux_h_unit_bound hk f ε hfar ((BLR_soundness_general_aux_h_split_sum hk f ε hfar)))
    -- Bound the non-unit part: (k − φ(k)) terms, each ≤ 1.
    let h_nonunit_bound : ∑ j ∈ Finset.filter (fun j => ¬IsUnit j) (Finset.univ : Finset (ZMod k)), ∑ s : ZkVec k n, ‖fourier_coeff (lift_omega_j j f) s‖ ^ 2 * (fourier_coeff (lift_omega_j j f) s).re ≤ (k - k.totient : ℝ) := (BLR_soundness_general_aux_h_nonunit_bound hk f ε hfar ((BLR_soundness_general_aux_h_split_sum hk f ε hfar)) h_unit_bound)
    -- Combine the two bounds and simplify the arithmetic.
    rw [ div_mul_eq_mul_div, div_mul_eq_mul_div, sub_div', div_mul_cancel₀ ] <;> first | positivity | nlinarith [((BLR_soundness_general_aux_h_split_sum hk f ε hfar))];
  · positivity
