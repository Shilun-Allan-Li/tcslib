...
private lemma termSubTree_foldl_numFree_lt {n : ℕ}
  ...
  -- ρ'.freeVars ⊆ ρ.freeVars (foldl only adds fixings)
  ...
  -- l.var ∈ ρ.freeVars but l.var ∉ ρ'.freeVars
  have hne : l.var ∉ ρ'.freeVars := by
    ...
    | cons hd tl ih =>
      ...
      · apply termSubTree_foldl_preserves_nonnone
        rw [show hd.var ∈ Restriction.freeVars ρ from hfree, if_pos rfl]
        simp [Function.update]
      · apply ih hl_tl; split
        · by_cases heq : l.var = hd.var
          · rw [heq]; simp [Restriction.freeVars, Finset.mem_filter,
              Function.update_apply, Option.isNone]
          · rw [Restriction.freeVars, Finset.mem_filter] at hfree ⊢
            simp only [Finset.mem_univ, true_and, Function.update_apply,
              Option.isNone_iff_eq_none, Ne.symm heq, ↓reduceIte] at hfree ⊢
            exact hfree
        · exact hfree
      ...
      · -- alive
        ...
        · rename_i t hfind
          ...
          · -- t fixed under ρ'
            ...
            exact fun l hl => Literal.fixedBy_eval_true l ρ' ht_fixed' x
          · -- t not fixed: recurse
    ...
    | cons step rest =>
      ...
      · -- find? = some t_clause
        ...
        | fl :: fls =>
          ...
          case hA' => -- pcl.2.1 v = none → σ_fold v = (encode ... σ []).1 v
            intro v hv'
            simp only [σ_fold_def]
            -- pcl.2.1 v = none → v not targeted by foldl → σ_fold v = σ_dec v
            have hv_free : ρ₀ v = none := by
              by_contra h; push_neg at h
              exact absurd hv' (processClauseLits_rho_ne_none _ _ ρ₀ σ v h)
            have hne_v : ∀ p ∈ (fl :: fls), p.1.var ≠ v := by
              intro p hp heq; exact absurd (heq ▸ hv')
                (processClauseLits_rho_ne_none_of_mem _ _ _ _ _ p hp heq)
            rw [foldl_sigma_stable t_clause _ _ _ (processClauseLits_aux_ne_nonfree
              t_clause _ _ ρ₀ σ v hmem_zip hne_v)]
            -- σ_dec v = enc.1 v = rec_enc.1 v = (encode ... σ []).1 v
            rw [hA v hv_free, hγ]
            -- rec_enc.1 v = (encode ... σ []).1 v by σ-independence at free v
            exact encode_go_fst_sigma_indep_at_free f w fuel pcl.1 pcl.2.1
              pcl.2.2.1 σ v hv' (by
                rw [processClauseLits_sigma_stable _ _ _ _ _ hne_v]; exact hE v hv_free)
              (hE v hv_free)
          case hB' => -- pcl.2.1 v = none → ρ₀_fold v = (encode ... σ []).1 v
            intro v hv'
            simp only [ρ₀_fold_def]
            have hv_free : ρ₀ v = none := by
              by_contra h; push_neg at h
            ...
            have hne_v : ∀ p ∈ (fl :: fls), p.1.var ≠ v := by
              intro p hp heq; exact absurd (heq ▸ hv')
                (processClauseLits_rho_ne_none_of_mem _ _ _ _ _ p hp heq)
            rw [foldl_rho_stable t_clause _ _ _ (processClauseLits_aux_ne_nonfree
              t_clause _ _ ρ₀ σ v hmem_zip hne_v)]
            rw [hB v hv_free, hγ]
            exact encode_go_fst_sigma_indep_at_free f w fuel pcl.1 pcl.2.1
              pcl.2.2.1 σ v hv' (by
                rw [processClauseLits_sigma_stable _ _ _ _ _ hne_v]; exact hE v hv_free)
              (hE v hv_free)
          case hfuel' => -- df ≥ (encode ... σ []).2.length + 1
            rw [← encode_go_snd_sigma_indep f w fuel pcl.1 pcl.2.1 σ pcl.2.2.1,
                ← hsigma_indep]
            rw [haux] at hfuel; simp [hpcl_cons, List.length_append] at hfuel ⊢; omega
          case henc' => rfl
          case hbase' => -- base case for recursive call
            intro heq
            rw [show (razborovEncode.go f w fuel pcl.1 pcl.2.1 σ []).2 = [] from by
              rw [← encode_go_snd_sigma_indep, ← hsigma_indep]; rw [heq]]
            have hA_rec : ∀ v, pcl.2.1 v = none → σ_fold v = σ v := by
              intro v hv'
              simp only [σ_fold_def]
              have hv_free : ρ₀ v = none := by
                by_contra h; push_neg at h
              ...
              rw [foldl_sigma_stable t_clause _ _ _ (processClauseLits_aux_ne_nonfree
                t_clause _ _ ρ₀ σ v hmem_zip hne_v)]
              rw [hA v hv_free, hγ, hrec_def,
                encode_go_fst_sigma_indep_at_free f w fuel pcl.1 pcl.2.1 pcl.2.2.1 σ v hv'
                  (by rw [processClauseLits_sigma_stable _ _ _ _ _ hne_v]; exact hE v hv_free)
                  (hE v hv_free)]
              exact congrFun (congrArg Prod.fst heq) v
            -- roundtrip_base for the recursive call
            apply roundtrip_base f w pcl.2.1 σ σ_fold ρ₀_fold df
            · exact fun v hv' => hE v (by
                by_contra h; exact absurd hv' (processClauseLits_rho_ne_none _ _ ρ₀ σ v h))
            · exact hA_rec
            · intro v hv'; simp only [σ_fold_def]
              by_cases hv : ρ₀ v = none
              · rw [processClauseLits_foldl_sigma_none t_clause _ _ ρ₀ σ _ _ hmem_zip hv
                  (by simp only [hpcl_def] at hv'; exact hv')]
                exact (hE v hv).symm
...
private lemma bad_count_bound {n : ℕ} (f : DNF n) (w s d : ℕ)
  ...
  set S := Finset.univ.filter fun ρ : Restriction n =>
      IsRestriction s ρ ∧ IsBadRestriction f.eval d ρ
  -- Map each bad restriction to γ = (razborovEncode f w d ρ).1
  set g : Restriction n → Restriction n := fun ρ => (razborovEncode f w d ρ).1
  -- Fiber bound: #{ρ ∈ S | g ρ = γ} ≤ (2w)^d for each γ
  have hfiber : ∀ γ ∈ S.image g,
      (S.filter (fun ρ => g ρ = γ)).card ≤ (2 * w) ^ d := by
    intro γ _
    apply le_trans (Finset.card_le_card _) (fiber_bound f w s d hw hd γ)
    intro ρ hρ
    obtain ⟨hρS, hρg⟩ := Finset.mem_filter.mp hρ
    obtain ⟨_, hρa, hρb⟩ := Finset.mem_filter.mp hρS
    exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, hρa, hρb, hρg⟩
  -- |S| ≤ (2w)^d · |image| by fiber decomposition
  have h1 : S.card ≤ (2 * w) ^ d * (S.image g).card :=
    Finset.card_le_mul_card_image S ((2 * w) ^ d) hfiber
  -- |image| ≤ C(n, s-d) · 2^(n-(s-d)) since γ is always a (s-d)-restriction
  have h2 : (S.image g).card ≤ n.choose (s - d) * 2 ^ (n - (s - d)) := by
    sorry
  calc S.card
      ≤ (2 * w) ^ d * (S.image g).card := h1
    _ ≤ (2 * w) ^ d * (n.choose (s - d) * 2 ^ (n - (s - d))) := by gcongr
    _ = n.choose (s - d) * 2 ^ (n - (s - d)) * (2 * w) ^ d := by ring
