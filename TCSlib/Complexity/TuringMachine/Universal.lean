...
private def universalActionBits {n : ℕ} (a : Action 1 Bool (Fin (n + 1))) :
    Fin 8 → Bool :=
...
private def universalNextOnes {n : ℕ} : Option (Fin (n + 1)) → ℕ
  | none => 0
  | some q => q.val + 1

/-- Fixed fields followed by one self-delimiting unary field form a record. -/
private lemma universal_record_shape {n : ℕ} (a : Action 1 Bool (Fin (n + 1))) :
...
private lemma universal_skip_fixed {x : List Bool}
...
private def universalSkipDone (dest : Option (Fin 9)) : UniversalControl :=
...
private lemma universal_live_block (M : CodeTM) (α : List Bool) {x : List Bool}
