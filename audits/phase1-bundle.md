...
# ATTACHMENT B — Lean sources under audit
...
An oracle Turing machine [AB09, §3.4, pulled forward to Chapter 1 to validate the model
architecture] is a multi-tape machine with one additional designated *query tape* and three
designated states `qQuery`, `qYes`, `qNo`. Whenever the machine enters `qQuery`, the string
currently written on the query tape is submitted to the oracle `O`: in a single step the
machine moves to `qYes` if the query is in `O` and to `qNo` otherwise, with all tapes and
heads unchanged.
...
* **The answer step** changes only the state; heads and tapes stay put. Some texts also
  erase the query tape; the two conventions simulate each other with constant overhead.
...
* `Turing.OracleTM` — the oracle machine. [AB09, Definition 3.6]
...
* [AB09] S. Arora, B. Barak, *Computational Complexity: A Modern Approach*,
  Cambridge University Press, 2009. (§3.4: oracle machines; Definition 3.6.)
-/
...
answer states. Finiteness of `State` is deferred exactly as for `MultiTapeTM`.
[AB09, Definition 3.6] -/
...
extended by three fresh states serving as `qQuery`, `qYes`, `qNo`, the transition
function acts as before on original states (never moving into the fresh states, and
ignoring the query tape), and halts immediately from the fresh states (which are
unreachable from the initial configuration). -/
